...
if field_type == "Диполь":
    M = st.sidebar.slider("Момент диполя M", 0.5, 5.0, 1.0, 0.1)
...
# Координатная сетка для стрелок (quiver)
x = np.linspace(-3, 3, grid_size)
y = np.linspace(-3, 3, grid_size)
X, Y = np.meshgrid(x, y)

# Более мелкая сетка для линий тока (streamplot)
...
if field_type == "Вихрь":
    ...
    r2 = X**2 + Y**2 + eps
    r4 = r2**2
    U = M * (Y**2 - X**2) / (2*np.pi * r4)
    V = M * (-2 * X * Y) / (2*np.pi * r4)
    title = f"Диполь (M = {M})"
    r2s = Xs**2 + Ys**2 + eps
    r4s = r2s**2
    Us = M * (Ys**2 - Xs**2) / (2*np.pi * r4s)
    Vs = M * (-2 * Xs * Ys) / (2*np.pi * r4s)
...
with st.expander("📘 О текущем течении"):
    if field_type == "Источник (сток)":
        ...
        **Анализ.** Вихреисточник — суперпозиция источника и вихря. Линии тока — логарифмические спирали. При $Q=0$ — вихрь, при $\\Gamma=0$ — источник.
        ...
        st.markdown(f"""
        **Диполь (ориентация по оси X, M = {M})**
        ...
        **Анализ.** Диполь возникает при сближении источника и стока равной интенсивности. Момент диполя $M$ задаётся пользователем. Модуль скорости $|\\vec{{v}}| \\sim M/r^{3}$. Линии тока — окружности, проходящие через начало координат, с центрами на оси $y$.
        """)
...
with st.expander("ℹ️ О приложении"):
    ...
    **Технологии:** Python, NumPy, Matplotlib, Streamlit.
    
    **Особенности реализации:**
    - Для стрелок (`quiver`) используется сетка до 50×50 точек (читаемость)
    - Для линий тока (`streamplot`) — сетка 100×100 точек (плавность)
    - Добавлена константа ε = 10⁻⁶ для избежания деления на ноль в особых точках
    """)

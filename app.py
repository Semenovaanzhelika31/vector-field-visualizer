...
if field_type == "Диполь":
    st.sidebar.markdown("💡 *Для диполя: увеличьте масштаб (50-150), чтобы стрелки стали короче*")
    arrow_scale = st.sidebar.slider("Масштаб стрелок (чем больше, тем короче)", 20, 150, 60, 5)
else:
...
if field_type == "Вихрь":
    ...
    r4 = r2**2
    M = 1.0  # момент диполя
    # Правильные формулы диполя, ориентированного по оси X
    U = M/(2*np.pi) * (Y**2 - X**2) / r4
    V = M/(2*np.pi) * (-2 * X * Y) / r4
    title = "Диполь (ось X)"
    r2s = Xs**2 + Ys**2 + eps
    r4s = r2s**2
    Us = M/(2*np.pi) * (Ys**2 - Xs**2) / r4s
    Vs = M/(2*np.pi) * (-2 * Xs * Ys) / r4s
...
with st.expander("📖 О текущем течении"):
    if field_type == "Источник/Сток":
        ...
        **{name} (Q = {Q})**
        ...
        - **Анализ:** {name} — радиальное течение.
        """)
        ...
        **Вихреисточник (Q = {Q}, Γ = {Gamma})**
        ...
        - **Анализ:** Спиралевидные линии тока.
        """)
    elif field_type == "Диполь":
        st.markdown(f"""
        **Диполь (ориентация по оси X)**
        ...
        - **Поле скоростей:** 
          - $u_x = \\frac{{M}}{{2\\pi}}\\frac{{y^2 - x^2}}{{(x^2+y^2)^2}}$
          - $u_y = -\\frac{{M}}{{2\\pi}}\\frac{{2xy}}{{(x^2+y^2)^2}}$
        - **Анализ:** Замкнутые линии тока — окружности, проходящие через начало координат.
        """)
    else:
        st.markdown(f"""
        **Равномерный поток (угол = {flow_angle}°)**
        ...
        - **Анализ:** Параллельные линии тока.
        """)

with st.expander("ℹ️ О приложении"):
    st.markdown("""
    **Правильные формулы диполя:**
    - $u_x = \\frac{M}{2\\pi}\\frac{y^2 - x^2}{(x^2+y^2)^2}$
    - $u_y = -\\frac{M}{2\\pi}\\frac{2xy}{(x^2+y^2)^2}$
    
    Линии тока диполя — **окружности**, проходящие через начало координат.
    Жёлтые стрелки (большая скорость) должны быть направлены **вдоль линий тока**.
    """)

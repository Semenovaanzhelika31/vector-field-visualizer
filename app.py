...
# Масштаб стрелок с пояснением
if field_type == "Диполь":
    st.sidebar.markdown("💡 *Для диполя: увеличьте масштаб (50-100), чтобы стрелки стали короче*")
...
# Для диполя используем `scale=None` и регулируем длину через `scale_units` и `width`
if field_type == "Диполь":
...
with st.expander("📖 О текущем течении"):
    if field_type == "Источник/Сток":
        ...
        **Диполь**
        ...
        - **Совет:** Покрутите слайдер "Масштаб стрелок" — увеличьте до 80-100, чтобы стрелки стали короче и аккуратнее.
        """)
    else:
        st.markdown(f"""
...
with st.expander("ℹ️ О приложении"):
    st.markdown("""
    **Как настроить отображение диполя:**
    - Стрелки в диполе могут быть слишком длинными по умолчанию
    - Увеличьте **«Масштаб стрелок»** до 80–100 — стрелки станут короче
    - Уменьшите масштаб — стрелки станут длиннее

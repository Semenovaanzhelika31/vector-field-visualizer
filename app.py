...
if field_type == "Вихрь":
    ...
    r2 = X**2 + Y**2 + eps
    r4 = r2**2
    # Правильные формулы диполя (ось X)
    U = (Y**2 - X**2) / r4
    V = (-2 * X * Y) / r4
    title = "Диполь (ось X)"
    r2s = Xs**2 + Ys**2 + eps
...
with st.expander("📖 О текущем течении"):
    if field_type == "Источник/Сток":
        ...
        **Диполь (ориентация по оси X)**
        ...
        - **Поле скоростей:** $u_x = \\frac{{M}}{{2\\pi}}\\frac{{y^2 - x^2}}{{(x^2+y^2)^2}},\\quad u_y = -\\frac{{M}}{{2\\pi}}\\frac{{2xy}}{{(x^2+y^2)^2}}$
